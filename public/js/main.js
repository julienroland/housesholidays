...
(function ($) {
    ...
    var sBasePath = '/',
        ...
        $homePaysSelect = $('#Country'),
        $homeRegionSelect = $('#Region'),
        $homeSousRegionSelect = $('#Province'),
    ...
    $(function () {
        ...
        if ($('#home').length > 0 || $('#listing').length > 0 ) {
            loadHomeRegion($homePaysSelect);
        }


        $('form[data-exist="true"] input[type="submit"]').on('click', validAndCheckForm);

        $homePaysSelect.on('change', function () {
            loadHomeRegion($(this));

        });

        $homeRegionSelect.on('change', function () {
            loadHomeSousRegion($(this));
        });

        $quickSearch.on('keypress', homeQuickSearch);

        $quickSearch.on('keydown', function () {
            homeQuickSearch($(this));
        });


        $quickSearch.on('click', function () {
            showHomeQuickSearch($(this));
        });
        $('.changeLang select').on('change', function () {
            $('.changeLang').submit();
        });
    ...
    var countFormDelay = function (sText) {

        var sLetters = sText.length;

        return 150 * sLetters;
    };

    var validAndCheckForm = function (e) {
        e.preventDefault();
        ...
        $.ajax({
            ...
            success: function (oData) {
                ...
                if (typeof oData.search_errors !== "undefined") {

                    $search_loader.hide();

                    $search_errors.find('li').remove();
                    $search_errors.append('<li>' + oData.search_errors + '</li>').fadeIn(550).delay(countFormDelay(oData.search_errors)).fadeOut(350);
                    $form.find('input[type="submit"]').removeAttr('disabled');
                }

                if (typeof oData.success !== "undefined") {
    ...
    var loadHomeSousRegion = function ($that) {
        var nVal = $that.val();

        $.ajax({
            type: "get",
            url: sBasePath + "getSousRegionFromRegion/" + nVal,
            success: function (oData) {
                var sData = '';
                $homeSousRegionSelect.find('option').remove();
                $.each(oData, function (key, value) {
                    sData = sData + '<option value="' + key + '">' + value + '</option>';
                });

                $homeSousRegionSelect.append(sData);

                if ($homeSousRegionSelect.attr('data-session') != 'null') {
                    $homeSousRegionSelect.find('option[value="' + $homeSousRegionSelect.attr('data-session') + '"]').attr('selected', 'selected');
                }
            }
        });
    };

    var loadHomeRegion = function ($that) {

        var nVal = $that.val();
        if (nVal != 0) {
            $.ajax({
                type: "get",
                url: sBasePath + "getRegionFromPays/" + nVal,
                success: function (oData) {
                    var sData = '';
                    $homeRegionSelect.find('option').remove();
                    $.each(oData, function (key, value) {
                        sData = sData + '<option value="' + key + '">' + value + '</option>';
                    });

                    $homeRegionSelect.append(sData);

                    if ($homeRegionSelect.attr('data-session') != 'null') {
                        $homeRegionSelect.find('option[value="' + $homeRegionSelect.attr('data-session') + '"]').attr('selected', 'selected');
                    }

                    loadHomeSousRegion($homeRegionSelect);
                }
            });
        }
    };
    ...
    var homeQuickSearch = function (e) {
        var $that = $(this);
        console.log(sBasePath);
        $.ajax({
            type: "get",
            url: sBasePath + "quick-search",
            data: $that.serialize(),
            success: function (oData) {
                displayHomeQuickSearch(oData);
            }
        });
        var key = e.which;
        switch (key) {
    ...
    var showHomeQuickSearch = function ($that) {
        $quickSearchResult.show();
    }

    var displayHomeQuickSearch = function (oData) {

        var sList = '';
        $quickSearchResult.find('div').remove();
        $.each(oData, function (key, value) {

            if (typeof value.pays.propriete_count_relation[0] !== "undefined") {

                var paysCount = value.pays.propriete_count_relation[0].count;
                var url = sBasePath + oLang.routes.locations_vacances + '/' + oLang.routes.location + '-' + value.nom;
                sList += '<div><a href="' + url + '">' + value.nom + ' (' + paysCount + ') </a></div>';
            }

            if (typeof value.pays.region !== "undefined") {
                $.each(value.pays.region, function (key2, value2) {
                    if (typeof value2.propriete_count_relation[0] !== "undefined") {

                        var regionCount = value2.propriete_count_relation[0].count;
                        var url2 = sBasePath + oLang.routes.locations_vacances + '/' + oLang.routes.location + '-' + value.nom + '/' + value2.region_traduction[0].nom;
                        sList += '<div><a href="' + url2 + '">' + value.nom + ' ~ ' + value2.region_traduction[0].nom + ' (' + regionCount + ')</a></div>';
                    }
        ...
        $quickSearchResult.append(sList);
    ...
    var appendTarif = function (oData) {
        ...
        for (var i in oData) {
            if (oData[i].prix_weekend != null) {
                var prix_weekend = oData[i].prix_weekend
            } else {
                var prix_weekend = ''
            }
    ...
    var defineBasePath = function(){
        sBasePath += prefix + '/';
    }

    var getLangPrefix = function () {

        prefix = $('html').attr('lang');
    }
            ...
            $.ajax({
                ...
                success: function (oData) {
                    ...
                    for (var i in oData) {
                        ...
                        sDataImg += '<li><div class="image"><img src="' + upload_dir + userId + '/' + propriete_dir + '/' + proprieteId + '/' + oData[i].url + '"></div><a href="" class="supprimerImage" data-id="' + oData[i].id + '" data-proprieteId="' + proprieteId + '" title="' + oLang.form.supp + '">' + oLang.form.supp_image + '</a></li>';


                    }

                    $('#images ul').append(sDataImg);
                    $('.supprimerImage').on('click', function (e) {
                        e.preventDefault();
                        deleteImage($(this));
    ...
    var uploadFile = function () {
        ...
        settingsUpload = $("#mulitplefileuploader").uploadFile({
            ...
            onSuccess: function (files, data, xhr) {
                ...
                getProprietePhoto($('form.upload').attr('data-userId'), $('form.upload').attr('data-proprieteId'));
            },
