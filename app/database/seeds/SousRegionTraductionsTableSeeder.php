...
class SousRegionTraductionsTableSeeder extends Seeder {

	public function run()
	{
		// Uncomment the below to wipe the table clean before populating
		// DB::table('sousregiontraductions')->truncate();
				...
				if(isset($region['subregions'])){
					...
					foreach($region['subregions'] as $subregions){
						...
						for($m=0; $m < count($lang); $m++){
							...
							array_push($sous_regions_traductions,['nom'=>$nom, 'sous_region_id'=>$p, 'langage_id'=>$m+1]);
						}
		...
		// Uncomment the below to run the seeder
		 DB::table('sous_regions_traductions')->insert($sous_regions_traductions);
	}
