			...
			<ul id="coordonee-proprietaire">
				<li><strong>{{trans('general.nom')}} :</strong><br />
					...
					<!-- 		@if(isset($maternelle) && Helpers::isOk($maternelle))
							...
							<ol>
								@foreach($user->langage as $langue)
								<li>
									<img src="{{Config::get('var.image_folder')}}flags/{{$langue->initial}}.png" alt="{{$langue->nom}}" title="{{$langue->nom}}">
								</li>
								@endforeach
							</ol>
						</li>
		...
		<div id="quisommesnous">
			<div class="tabs">
				...
				<ul class="tabs">
					...
					<li><a href="#tabs-2" class="localisation"><span>Localisation</span></a></li>
					<li><a href="#tabs-3"><span>Disponibilitées</span></a></li>
					@if(Helpers::isOk( $propriete->video ))
					<li><a href="#tabs-4"><span>Video</span></a></li>
					@endif
