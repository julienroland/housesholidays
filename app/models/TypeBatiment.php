...
class TypeBatiment extends Eloquent {
    ...
    public static function listing($default = null)
    {
        $typeBatimentList = new Collection(TypeBatimentTraduction::where(Config::get('var.lang_col'), Session::get('langId'))->orderBy('nom', 'asc')->lists('nom'));

        if (Helpers::isOk($typeBatimentList))
        {
            $typeBatimentList->prepend($default);
        }
    ...
    public
    static function getListForm($orderBy = 'nom', $orderWay = 'asc')
    {
